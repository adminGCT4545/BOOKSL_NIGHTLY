...
app.get('/api/chat/stream', (req, res) => {
  ...
  const data = JSON.stringify({
    content: "Welcome to the BookSL Train Dashboard. The AI assistant functionality has been removed. This is a placeholder response.",
    timestamp: new Date()
  });
...
// Basic root endpoint
app.get('/', (req, res) => {
  res.send('BookSL Train Dashboard Backend is running!');
});
...
if (process.argv[1] === __filename) {
  const server = app.listen(port, () => {
    console.log(`BookSL Train Dashboard Backend listening at http://localhost:${port}`);
